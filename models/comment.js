...
class Comment extends Sequelize.Model {
  static initiate(sequelize) {
    Comment.init({
      ...
      parent_comment_id: { 
        type: Sequelize.INTEGER, // 부모 댓글 ID (NULL은 일반 댓글, 값이 있으면 대댓글)
        allowNull: true,
        defaultValue: null,
      },
    }, {
      ...
      modelName: 'Comment',
      tableName: 'comment',
      paranoid: true, // 삭제 활성화 (deletedAt)
      charset: 'utf8',
  ...
  static associate(db) {
    ...
    db.Comment.hasMany(db.Comment, { 
      ...
      as: 'Replies',  // 대댓글을 가져올 때 Replies로 조회 가능
      onDelete: 'CASCADE'
    });
    db.Comment.belongsTo(db.Comment, { 
      foreignKey: 'parent_comment_id', 
      as: 'ParentComment'  // 부모 댓글을 가져올 때 ParentComment로 조회 가능
    });
